...
**v0.1.2 — released.** Five binaries: macOS (Apple Silicon + Intel), Linux (x86_64 + ARM64, AppImage + .deb), Windows x86_64 (.msi + .exe). The bundled Rust DPI engine ships inside each binary — no libpcap, no Npcap, no Wireshark install required. Auto-updater lands in v0.3.0. See [CHANGELOG](CHANGELOG.md) and [Roadmap](#roadmap) below.
